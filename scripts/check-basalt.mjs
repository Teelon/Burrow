 ...
 */
const patterns = [/rounded-/, /shadow-(xs|sm|ml|md|lg|xl|2xl)/, /bg-neutral-/, /border-neutral-/, /text-neutral-/, /dark:/, /TODO\(ui\)/, /once available/]
...
for (const file of walk(dir)) {
  ...
  lines.forEach((line, i) => {
    for (const re of patterns) {
      if (re.test(line)) {
        console.error(`${path.relative(root, file)}:${i + 1}: forbidden ${re.source}: ${line.trim()}`)
        violations++
      }
    }
  })
