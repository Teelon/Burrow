...
export function run(cmd, args = [], opts = {}) {
  const { dryRun = false, capture = false, cwd = root, env, allowFail = false, quiet = false } = opts
  const printable = `${path.isAbsolute(cmd) ? path.relative(root, cmd) : cmd} ${args.join(' ')}`.trim()
  if (dryRun) {
