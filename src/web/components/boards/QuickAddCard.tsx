...
export function QuickAddCard({
  ...
  const handleSave = async (keepOpen = false) => {
    ...
    const priorityMatch = cleanTitle.match(/(?:^|\s)\/(urgent|high|medium|low|priority\s+(urgent|high|medium|low))(?:\s|$)/i)
    ...
    <div className="rounded-md border border-[var(--line)] bg-[var(--surface)] text-[var(--text)] p-2.5 space-y-2 relative shadow-md">
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {m.name && m.email && (
                      <div className="text-[10px] text-[var(--muted)] truncate mt-0.5">{m.email}</div>
                    )}
                  </div>
                </button>
              )
      ...
      {menuMode === 'hash' && (
        <div className="absolute left-0 bottom-full mb-1.5 w-64 bg-[var(--surface)] border border-[var(--line)] rounded-md shadow-2xl p-1.5 z-50 text-xs space-y-0.5 max-h-56 overflow-y-auto">
          ...
          {menuQuery.trim() && !filteredTags.some((t) => t.name.toLowerCase() === menuQuery.toLowerCase()) && (
            <button
              type="button"
              onMouseDown={(e) => {
                e.preventDefault()
          ...
          {filteredTags.length === 0 && !menuQuery.trim() && (
            <div className="px-2 py-2 text-xs text-[var(--muted)] italic">No tags yet. Type to create one!</div>
          )}
        </div>
      )}
      ...
      <div className="flex items-center justify-between text-[10px] text-[var(--muted)] pt-1 border-t border-[var(--hair)] gap-2">
        <span className="truncate">↵ Enter to save &bull; ⇧↵ to add another &bull; ⌫ clears chips</span>
