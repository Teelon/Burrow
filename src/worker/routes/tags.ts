...
export const tagsRoutes = new Hono<Env>()
  .get('/api/projects/:pid/tags', requireSession, async (c) => {
  ...
  .post(
    ...
    async (c) => {
      ...
      const [existing] = await db
        .select()
        .from(t.tags)
        .where(and(eq(t.tags.projectId, projectId), eq(t.tags.name, name)));
      if (existing) {
  ...
  .patch(
    ...
    async (c) => {
      ...
      const [updated] = await db.select().from(t.tags).where(eq(t.tags.id, tagId));

      return c.json(updated);
    },
  )
  .delete('/api/tags/:id', requireSession, requireRole('editor'), async (c) => {
    const db = createDb(c.env.DB);
    const workspaceId = c.get('workspaceId');
    const tagId = c.req.param('id');
