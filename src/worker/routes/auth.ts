...
export const authRoutes = new Hono<Env>()
  .use('/api/auth/sign-in/*', rateLimiter({ max: 20, windowMs: 60_000 }))
  .use('/api/auth/sign-up/*', rateLimiter({ max: 20, windowMs: 60_000 }))
  .post('/api/auth/sign-up/email', async (c) => {
    ...
    const [wsCountResult] = await db.select({ count: sql<number>`count(*)` }).from(t.workspaces);
    const isFirstWorkspace = (wsCountResult?.count ?? 0) === 0;

    let inviteToAccept: { id: string; workspaceId: string; role: 'editor' | 'viewer' } | null =
      null;
    ...
    if (isFirstWorkspace) {
      ...
      await runBatch(db, [
        db.update(t.invites).set({ acceptedAt: now }).where(eq(t.invites.id, inviteToAccept.id)),
        db.insert(t.members).values({
