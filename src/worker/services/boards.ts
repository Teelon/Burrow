...
export async function createBoard(db: DB, args: CreateBoardArgs) {
  ...
  const [last] = await db
    ...
    .from(t.boards)
    .where(and(eq(t.boards.projectId, args.projectId), isNull(t.boards.deletedAt)))
...
export async function softDeleteBoard(db: DB, workspaceId: string, boardId: string) {
  const [board] = await db
    .select()
    .from(t.boards)
    .where(
  ...
  const statements: BatchItem<'sqlite'>[] = [
    db.update(t.boards).set({ deletedAt: now, updatedAt: now }).where(eq(t.boards.id, boardId)),
  ];
...
export async function restoreBoard(db: DB, workspaceId: string, boardId: string) {
  const [board] = await db
    .select()
    .from(t.boards)
    .where(and(eq(t.boards.id, boardId), eq(t.boards.workspaceId, workspaceId)));
  ...
  const cards = await db
    ...
    .from(t.cards)
    .innerJoin(t.notepads, eq(t.notepads.id, t.cards.notepadId))
    .where(and(eq(t.cards.boardId, boardId), eq(t.notepads.deletedAt, deleteTimestamp)));

  const notepadIds = cards.map((c) => c.id);

  const statements: BatchItem<'sqlite'>[] = [
    db.update(t.boards).set({ deletedAt: null, updatedAt: now }).where(eq(t.boards.id, boardId)),
  ];
...
export async function permanentDeleteBoard(db: DB, workspaceId: string, boardId: string) {
  const [board] = await db
    .select()
    .from(t.boards)
    .where(and(eq(t.boards.id, boardId), eq(t.boards.workspaceId, workspaceId)));
  ...
  if (notepadIds.length > 0) {
    ...
    for (const chunk of chunkByParamBudget(notepadIds, 1, 0)) {
      statements.push(db.delete(t.notepads).where(inArray(t.notepads.id, chunk)));
    }
  }
...
export async function deleteColumn(
  ...
  if (count > 0) {
    ...
    const [dest] = await db
      .select({ id: t.boardColumns.id })
      .from(t.boardColumns)
      .where(and(eq(t.boardColumns.id, moveToColumnId), eq(t.boardColumns.boardId, col.boardId)));
    if (!dest) {
