...
import { getDescendantIds, getNotepadDepth, getSubtreeHeight, wouldCauseCycle } from '../lib/tree';
...
export async function moveNotepad(db: DB, args: MoveNotepadArgs) {
  const [notepad] = await db
    .select()
    .from(t.notepads)
    .where(and(eq(t.notepads.id, args.notepadId), eq(t.notepads.workspaceId, args.workspaceId)));
  ...
  if (notepad.kind === 'card') {
    throw new HttpError(
      400,
      'cannot_move_card_notepad',
      'Cards cannot be moved in the notepad tree',
    );
  }
...
export async function restoreNotepad(db: DB, args: RestoreNotepadArgs) {
  ...
  const matchingNotepads = await db
    ...
    .from(t.notepads)
    .where(
      and(eq(t.notepads.projectId, notepad.projectId), eq(t.notepads.deletedAt, deleteTimestamp)),
    );
  ...
  for (const item of matchingNotepads) {
    statements.push(db.run(ftsInsertNowStmt(item.id, item.title, extractPlainText(item.content))));
  }
...
export async function permanentDeleteNotepad(db: DB, args: PermanentDeleteNotepadArgs) {
  const [notepad] = await db
    .select()
    .from(t.notepads)
    .where(and(eq(t.notepads.id, args.notepadId), eq(t.notepads.workspaceId, args.workspaceId)));
...
export async function claimLock(db: DB, args: ClaimLockArgs) {
  ...
  const result = await adapter.acquire(args.notepadId, args.userId, args.clientId, LOCK_TTL_MS, {
