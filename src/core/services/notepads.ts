...
export class NotepadService {
  ...
  async createNotepad(args: CreateNotepadArgs): Promise<CreatedNotepad> {
    ...
    if (args.parentId) {
      ...
      const parentDepth = await getDepth(
        (id) => this.repos.notepads.getParent(id),
        parent.id,
      )
  ...
  async moveNotepad(args: MoveNotepadArgs): Promise<{ parentId: string | null; position: string }> {
    const notepad = await this.repos.notepads.findByIdAndWorkspace(args.notepadId, args.workspaceId)
    ...
    if (targetParentId) {
      if (
        await wouldCauseCycle(
          (rootId) => getDescendantIds((parentIds) => this.repos.notepads.getChildren(parentIds), rootId),
          args.notepadId,
          targetParentId,
        )
      ) {
      ...
      if (!parent || parent.projectId !== notepad.projectId || parent.deletedAt !== null || parent.kind !== 'notepad') {
    ...
    if (!args.afterId) {
      ...
      if (afterIndex === -1) throw badRequest('invalid_after_id', 'afterId not found among siblings')
  ...
  async softDeleteNotepad(args: SoftDeleteNotepadArgs): Promise<void> {
    ...
    if (notepad.kind === 'card') {
      throw badRequest('cannot_delete_card_notepad', 'Card notepads must be deleted via card service')
    }
  ...
  async restoreNotepad(args: RestoreNotepadArgs): Promise<void> {
    ...
    if (notepad.kind === 'card') {
      throw badRequest('cannot_restore_card_notepad', 'Card notepads must be restored via card service')
    }
  ...
  async permanentDeleteNotepad(args: PermanentDeleteNotepadArgs): Promise<void> {
    ...
    const allRows = await Promise.all(
      allIds.map((id) => this.repos.notepads.findById(id)),
    )
    const r2Keys = allRows
      .map((r) => r?.coverKey)
  ...
  async saveContent(args: SaveContentArgs): Promise<SaveContentResult> {
    ...
    if (lock && lock.expiresAt > now && (lock.userId !== args.actorId || (args.clientId && lock.clientId !== args.clientId))) {
      const holderName = 'Someone' // Would need user repo to get name
      throw conflict('locked', JSON.stringify({
        code: 'locked',
        holder: {
          userId: lock.userId,
          name: holderName,
