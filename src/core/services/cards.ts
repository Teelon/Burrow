...
export class CardService {
  ...
  async getCard(workspaceId: string, cardId: string) {
    const card = await this.repos.cards.findByIdAndWorkspace(cardId, workspaceId)
    if (!card) throw notFound('not_found', 'Card not found')

    // The worker route composed additional data (board, column, assignees, tags, subtasks, lock)
    // For the service, we return the base card and let the route compose if needed,
    // or we add a richer getCard method that does the composition.
    return card
  }
  ...
  async updateCard(
    ...
    if (updates.title !== undefined) {
      ...
      if (notepad) {
        ...
        await this.repos.notepads.insertFts(card.notepadId, newTitle, extractPlainText(notepad.content))
      }
    }
  ...
  async permanentDeleteCard(workspaceId: string, cardId: string): Promise<{ ok: true; cardId: string }> {
    const card = await this.repos.cards.findByIdAndWorkspace(cardId, workspaceId)
  ...
  async createSubtask(workspaceId: string, cardId: string, title: string): Promise<CardSubtask> {
    ...
    const [last] = await this.repos.cards.listSubtasks(cardId).then((s) => [s[s.length - 1]]).catch(() => [null as any])
  ...
  async deleteSubtask(workspaceId: string, cardId: string, subtaskId: string): Promise<{ ok: true; subtaskId: string }> {
    await this.requireCardForWorkspace(workspaceId, cardId)
    const subtasks = await this.repos.cards.listSubtasks(cardId)
  ...
  private async validateAssignees(workspaceId: string, userIds: string[]): Promise<{ userId: string }[]> {
