...
import { cards, cardSubtasks, cardAssignees, cardComments, boardColumns, boards, notepads, projects, members, tags, notepadTags, user } from '../schema'
...
export class PostgresCardRepository implements ICardRepository {
  ...
  async setAssignees(cardId: string, userIds: string[]): Promise<void> {
    ...
    if (userIds.length > 0) {
      await this.db.insert(cardAssignees).values(
        userIds.map((userId) => ({ cardId, userId }))
      )
    }
  }
  ...
  async setTagIds(notepadId: string, tagIds: string[]): Promise<void> {
    ...
    if (tagIds.length > 0) {
      await this.db.insert(notepadTags).values(
        tagIds.map((tagId) => ({ notepadId, tagId }))
      )
    }
  }
  ...
  async getMyTasks(
    ...
    const whereConditions = [eq(cardAssignees.userId, userId), eq(projects.workspaceId, workspaceId)]
  ...
  async getCardsSummary(workspaceId: string, cardIds: string[]): Promise<CardSummary[]> {
    ...
    const result = await this.db
      ...
      .groupBy(cards.id, cards.notepadId, notepads.title, cards.boardId, boards.name, cards.columnId, boardColumns.name, cards.priority, cards.dueDate)
