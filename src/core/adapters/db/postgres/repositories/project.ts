...
export class PostgresProjectRepository implements IProjectRepository {
  ...
  async updatePosition(id: string, position: string): Promise<void> {
    await this.db.update(projects).set({ position, updatedAt: Date.now() }).where(eq(projects.id, id))
  }

  async delete(id: string): Promise<void> {
    await this.db.update(projects).set({ archivedAt: Date.now(), updatedAt: Date.now() }).where(eq(projects.id, id))
  }
