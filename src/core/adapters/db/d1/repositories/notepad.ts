  ...
  return {
    ...
    async restore(id: string, deleteTimestamp: number): Promise<RestoredNotepad[]> {
      ...
      const matchingNotepads = await db
        ...
        .from(t.notepads)
        .where(and(eq(t.notepads.projectId, notepad.projectId), eq(t.notepads.deletedAt, deleteTimestamp)))
      ...
      for (const chunk of chunkByParamBudget(matchingIds, 1, 1)) {
        statements.push(
          db.update(t.notepads).set({ deletedAt: null, updatedAt: now }).where(inArray(t.notepads.id, chunk)),
        )
      }
      ...
      if (newParentId !== notepad.parentId || newPosition) {
        statements.push(
          db
            .update(t.notepads)
            .set({ parentId: newParentId, position: newPosition ?? notepad.position, updatedAt: now })
            .where(eq(t.notepads.id, id)),
        )
      }
      ...
      for (const item of matchingNotepads) {
        statements.push(
          db.run(
            ftsInsertNowStmt(item.id, item.title, extractPlainText(item.content)),
          ),
        )
      }
    ...
    async getContent(id: string): Promise<NotepadContent | null> {
      const [row] = await db
        .select({ id: t.notepads.id, workspaceId: t.notepads.workspaceId, title: t.notepads.title, content: t.notepads.content, version: t.notepads.version })
        .from(t.notepads)
        .where(eq(t.notepads.id, id))
      if (!row) return null
      return { id: row.id, workspaceId: row.workspaceId, title: row.title, content: row.content, version: row.version }
    },
    ...
    async isValidLink(link: { targetType: string; targetId: string }): Promise<boolean> {
      ...
      if (link.targetType === 'notepad') {
        const [row] = await db.select({ id: t.notepads.id }).from(t.notepads).where(eq(t.notepads.id, link.targetId))
        return !!row
      }
      if (link.targetType === 'card') {
        const [row] = await db.select({ id: t.cards.id }).from(t.cards).where(eq(t.cards.id, link.targetId))
        return !!row
      }
      if (link.targetType === 'board') {
        const [row] = await db.select({ id: t.boards.id }).from(t.boards).where(eq(t.boards.id, link.targetId))
        return !!row
      }
      if (link.targetType === 'user') {
        const [row] = await db.select({ id: t.user.id }).from(t.user).where(eq(t.user.id, link.targetId))
        return !!row
      }
      return false
    },
    ...
    async getLock(notepadId: string): Promise<EditLock | null> {
      ...
      return row ? { notepadId: row.notepadId, userId: row.userId, clientId: row.clientId, expiresAt: row.expiresAt } : null
    },
    ...
    async insertFtsConditional(notepadId: string, title: string, body: string, version: number, content: string): Promise<void> {
    ...
    async listLinks(sourceId: string): Promise<StoredLink[]> {
      ...
      return rows.map((r) => ({ targetType: r.targetType as StoredLink['targetType'], targetId: r.targetId }))
    },

    async replaceLinks(sourceId: string, added: StoredLink[], removed: StoredLink[], version: number, content: string): Promise<void> {
      const cond = liveContentCond(sourceId, version, content)
      const statements: BatchItem<'sqlite'>[] = [
        ...linkDeleteStatements(db, { sourceId, removed, cond, condParams: LIVE_CONTENT_COND_PARAMS }),
        ...linkInsertStatements(db, { sourceId, added, cond, condParams: LIVE_CONTENT_COND_PARAMS }),
      ]
    ...
    async getLastRootPosition(projectId: string): Promise<string | null> {
      const [last] = await db
        .select({ position: t.notepads.position })
        .from(t.notepads)
        .where(and(eq(t.notepads.projectId, projectId), isNull(t.notepads.parentId), isNull(t.notepads.deletedAt), eq(t.notepads.kind, 'notepad')))
        .orderBy(desc(t.notepads.position))
        .limit(1)
      return last ? positionAfterLast(last.position) : null
    },
    ...
    // Search/suggest
    async findByPattern(projectId: string, pattern: string, limit: number): Promise<{ id: string; title: string; icon: string | null }[]> {
      const searchPattern = `%${pattern}%`
      const rows = await db
        .select({ id: t.notepads.id, title: t.notepads.title, icon: t.notepads.icon })
        .from(t.notepads)
        .where(and(eq(t.notepads.projectId, projectId), isNull(t.notepads.deletedAt), sql`${t.notepads.title} LIKE ${searchPattern}`))
...
async function extractAndFilterLinks(db: DB, workspaceId: string, content: string): Promise<{ validLinks: StoredLink[] }> {
  const { extractFromContent } = await import('../../../../shared/extract')
  const extraction = extractFromContent(content)
  const validLinks = await filterWorkspaceTargets(db, workspaceId, extraction.links as StoredLink[])
  return { validLinks }
}
...
function buildSaveContentStatements(
  ...
  return [
    ...
    ...linkDeleteStatements(db, { sourceId: row.id, removed, cond, condParams: LIVE_CONTENT_COND_PARAMS }),
    ...linkInsertStatements(db, { sourceId: row.id, added, cond, condParams: LIVE_CONTENT_COND_PARAMS }),
  ]
}
