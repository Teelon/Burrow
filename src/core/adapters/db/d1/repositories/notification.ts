  ...
  return {
    async listByUser(userId: string, workspaceId: string, unreadOnly: boolean): Promise<Notification[]> {
      const rows = await db
        ...
        .where(and(eq(t.notifications.userId, userId), eq(t.notifications.workspaceId, workspaceId), unreadOnly ? isNull(t.notifications.readAt) : undefined))
    ...
    async markRead(userId: string, notificationIds: string[]): Promise<void> {
      ...
      if (notificationIds.length === 0) {
        await db.update(t.notifications).set({ readAt: now }).where(and(eq(t.notifications.userId, userId), isNull(t.notifications.readAt)))
        return
      }
      const statements: BatchItem<'sqlite'>[] = []
      for (const chunk of chunkInList(notificationIds, 2)) {
        statements.push(
          db.update(t.notifications).set({ readAt: now }).where(and(eq(t.notifications.userId, userId), inArray(t.notifications.id, chunk))),
        )
      }
    ...
    async createMentions(args: { workspaceId: string; notepadId: string; cardId: string | null; actorId: string; userIds: string[]; now: number }): Promise<void> {
