...
export function createProjectRepository(db: DB): IProjectRepository {
  return {
    ...
    async updatePosition(id: string, position: string): Promise<void> {
      await db.update(t.projects).set({ position, updatedAt: Date.now() }).where(eq(t.projects.id, id))
    },

    async delete(id: string): Promise<void> {
      // Soft delete by archiving
      await db.update(t.projects).set({ archivedAt: Date.now(), updatedAt: Date.now() }).where(eq(t.projects.id, id))
    },
