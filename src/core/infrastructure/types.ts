...
// ---------------------------------------------------------------------------
// Repository Interfaces (shapes derived from current worker services)
// ---------------------------------------------------------------------------
...
export interface ICardRepository {
  ...
  validateAssignees(workspaceId: string, userIds: string[]): Promise<{ userId: string }[]>
  // Tags
  ...
  // Search/suggest
  findByPattern(projectId: string, pattern: string, limit: number): Promise<{ id: string; title: string; boardName: string; priority: CardPriority | null }[]>
}
...
export interface INotepadRepository {
  ...
  // Tree navigation (for core services)
  getChildren(parentIds: string[]): Promise<{ id: string; parentId: string | null }[]>
  getParent(id: string): Promise<{ id: string; parentId: string | null } | null>
  // Link validation
  isValidLink(link: { targetType: string; targetId: string }): Promise<boolean>
  // Tags
  listTagIds(notepadId: string): Promise<string[]>
  setTagIds(notepadId: string, tagIds: string[]): Promise<void>
  // Locks (delegated to lock adapter, but repo may need to read lock row)
  getLock(notepadId: string): Promise<EditLock | null>
  // FTS (delegated to search adapter, but repo may need direct FTS ops)
  ...
  // Search/suggest
  findByPattern(projectId: string, pattern: string, limit: number): Promise<{ id: string; title: string; icon: string | null }[]>
}
...
export interface ITagRepository {
  ...
  validateByProject(projectId: string, tagIds: string[]): Promise<{ id: string }[]>
  findByPattern(projectId: string, pattern: string, limit: number): Promise<{ id: string; name: string; color: string | null }[]>
}
...
export interface INotificationRepository {
  ...
  createMentions(args: { workspaceId: string; notepadId: string; cardId: string | null; actorId: string; userIds: string[]; now: number }): Promise<void>
}
...
export interface IMemberRepository {
  ...
  findByUserIds(workspaceId: string, userIds: string[]): Promise<{ userId: string }[]>
  findByPattern(workspaceId: string, pattern: string, limit: number): Promise<{ userId: string; name: string; email: string; image: string | null }[]>
...
// ---------------------------------------------------------------------------
// Entity Types (minimal shapes matching the database schema)
// ---------------------------------------------------------------------------
