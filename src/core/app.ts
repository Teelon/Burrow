...
export function createCoreApp(infra: Infrastructure) {
  ...
  app.delete('/api/notepads/:id', requireSession, requireRole('editor'), async (c) => {
    const workspaceId = c.get('workspaceId');
    const notepadId = c.req.param('id');
  ...
  app.post('/api/notepads/:id/restore', requireSession, requireRole('editor'), async (c) => {
    const workspaceId = c.get('workspaceId');
    const notepadId = c.req.param('id');
  ...
  app.delete('/api/notepads/:id/permanent', requireSession, requireRole('owner'), async (c) => {
    const workspaceId = c.get('workspaceId');
    const notepadId = c.req.param('id');
  ...
  app.delete('/api/members/:userId', requireSession, requireRole('owner'), async (c) => {
    const workspaceId = c.get('workspaceId');
    const actorUserId = c.get('userId');
    const targetUserId = c.req.param('userId');
  ...
  app.delete('/api/tags/:id', requireSession, requireRole('editor'), async (c) => {
    const workspaceId = c.get('workspaceId');
    const tagId = c.req.param('id');
  ...
  app.get('/api/search', requireSession, zValidator('query', searchSchema), async (c) => {
    const workspaceId = c.get('workspaceId');
  ...
  app.get('/api/suggest', requireSession, zValidator('query', suggestQuerySchema), async (c) => {
    const workspaceId = c.get('workspaceId');
