...
import {
  cardCreationStatements,
  createCard,
  type CardPlan,
} from '../../src/worker/services/cards'
import {
  buildSaveContentStatements,
  saveContent,
} from '../../src/worker/services/notepads'
...
async function notepadRow(
  id: string,
): Promise<{ version: number; content: string } | null> {
  return env.DB
    .prepare('SELECT version, content FROM notepads WHERE id = ?')
    .bind(id)
...
async function ftsMatches(term: string): Promise<string[]> {
  const res = await env.DB.prepare(
    'SELECT notepad_id FROM notepads_fts WHERE notepads_fts MATCH ?',
  )
    .bind(term)
...
describe('D1 batch atomicity spike', () => {
  it("(a) a valid save batch commits content, FTS, links and notifications together", async () => {
    ...
    expect(links.results).toEqual([
      { target_type: 'user', target_id: seed.member2Id },
    ])
  ...
  it("(b) a failing final statement rolls back the whole save batch, including FTS, links and notifications", async () => {
    ...
    await expect(runBatch(db, statements)).rejects.toThrow(
    ...
    expect(
      await countRows(
        'SELECT COUNT(*) AS n FROM notepad_links WHERE source_id = ?',
        notepad.id,
      ),
    ).toBe(0)
    expect(
      await countRows(
        'SELECT COUNT(*) AS n FROM notifications WHERE notepad_id = ?',
        notepad.id,
      ),
    ).toBe(0)
  ...
  it("(c) createCard is atomic: success writes every row, a failing statement writes none of them", async () => {
    ...
    const card = await env.DB.prepare(
      'SELECT notepad_id FROM cards WHERE id = ?',
    )
      .bind(created.cardId)
    ...
    await expect(runBatch(db, failing)).rejects.toThrow(
      /UNIQUE constraint failed/,
    )
    ...
    expect(
      await env.DB
        .prepare('SELECT id FROM notepads WHERE id = ?')
        .bind(plan.notepadId)
        .first(),
    ).toBeNull()
    expect(
  ...
  it("(d) foreign keys are enforced and ON DELETE CASCADE removes dependent rows", async () => {
    ...
    await expect(
      orphan.run(),
      'D1 must reject inserts that violate a foreign key',
    ...
    await env.DB
      .prepare('DELETE FROM notepads_fts WHERE notepad_id NOT IN (SELECT id FROM notepads)')
      .run()
    await expectInvariantsHold(env.DB)
  })
